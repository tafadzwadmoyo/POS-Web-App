...
app.controller('invoiceController', function($scope) {
    ...
    firebase.database().ref('/invoices/' + $scope.$parent.invoiceId).once('value', ).then(function(invoice) {
        ...
        $scope.invoice.items_og = invoice.val().items;
    });
    ...
    $scope.save = () => {
        ...
        for (i in $scope.invoice.items) {
            ...
            updates['/products/' + i + "/stock"] = $scope.$parent.$parent.productsJSON[i].stock - ($scope.invoice.items[i]['qty'] - $scope.invoice.items_og[i]['qty'])
        }
