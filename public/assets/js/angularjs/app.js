...
var checkLogin = ($scope) => {
    $('#menu_li').show();
    $('#bottom_menu').show();
    var user = firebase.auth().currentUser;
    if (user || ($scope.$parent.title && $scope.$parent.title == 'Login')) {
        // User is signed in.
...
app.controller('posController', function($scope) {
    ...
    var getUrls = (arr, i) => {
        ...
        firebase.storage().ref('images/').child(arr[i]['imgId']).getDownloadURL().then(function(url) {
            // Or inserted into an <img> element:
...
app.config(function($stateProvider, $urlRouterProvider) {
    ...
    var clientLoginState = {
        name: 'Client Login',
        url: "/client",
        templateUrl: 'assets/html/client_login.html',
        controller: function($scope, $stateParams) {
            $scope.$parent.title = 'Client Zone';
            $scope.clientId = $stateParams.id;
            resizeMenu();
    ...
    $stateProvider.state(clientLoginState);
